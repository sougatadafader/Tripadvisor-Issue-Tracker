                            ...
                            <div className="row">
                                ...
                                {this.state.issue.user !==undefined ?
                                    ...
                                    className="d-block text-gray-dark">{this.state.issue['user']['login']}</strong></a>
                                        <div style={{fontSize:8}}>{new Date(this.state.issue.created_at).toDateString()}, {this.utilityservice.dateFormatter(this.state.issue.created_at)}</div>
                                        ...
                                        {this.state.comments.map((comment, id) =>
                                            <div className="media text-muted pt-3">
                                                ...
                                                <p className="media-body pb-3 mb-0 small lh-125 border-bottom border-gray">
                                                    ...
                                                    <div style={{fontSize:8}}>{new Date(comment.created_at).toDateString()}, {this.utilityservice.dateFormatter(comment.created_at)}</div>
                                                    {comment['body']}
                                                </p>
                                            </div>)
                                        }
                                    </div> : <div style={{width:'100%'}}><h4 className="mt-5 text-center">No comments yet</h4></div>
                                }
                            </div>
                            <hr/>
